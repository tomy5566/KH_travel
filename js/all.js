...
console.log('travelData[0].Name', travelData[0].Name);
console.log('travelData.length', travelData.length);
...
function updatecardlist() {
  ...
  // var strInfo = "";
  for (var i = 0; i < travelData.length; i++) {
      const strphoto = '<div class="cardphoto" style="background-image: url(' + travelData[i].Picture1 + ')">'+'<h2 class="cardname"> 景點:' + travelData[i].Name +'</h2></div>';
      const strInfo = '<div class="cardinfo"> '+ '<ul><li>地址:' +travelData[i].Add +'</li>'+' <li>電話:' +travelData[i].Tel +'</li></ul></div>';
